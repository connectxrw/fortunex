...
export const PromptInputAddAttachmentsButton = ({
  label = "Add photos or files",
  ...props
}: PromptInputActionAddAttachmentsProps & ComponentProps<typeof Button>) => {
  const attachments = usePromptInputAttachments();
    ...
    if (
      ...
      speechRecognition.onresult = (event) => {
        ...
        if (finalTranscript && textareaRef?.current) {
          ...
          const currentValue = controller?.textInput.value ?? textarea.value;
          const newValue =
            currentValue + (currentValue ? " " : "") + finalTranscript;

          // If using controller (PromptInputProvider), update state directly
          if (controller) {
            controller.textInput.setInput(newValue);
          } else {
            // Otherwise, update textarea directly
